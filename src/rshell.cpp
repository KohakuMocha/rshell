...
bool test(vector<string>& shadow)
{
    //Initialize struct
    struct stat attributes;
    if(shadow.at(0) != "test") {
	    if(shadow.at(0) == "[" && shadow.at(shadow.size() - 1 ) != "]")
	    {
	    	cout << "-rshell: [: missing `]\'" << endl;
	    	return false;
	    }
	    else shadow.pop_back();
    }
...
bool functionality (vector<string> cmds, unsigned &j) {
	    ...
	    for(; j < cmds.size(); ++j)
	    {
	    	...
	    	if(cmds.at(j) != ")" ) set.push_back(cmds.at(j));
